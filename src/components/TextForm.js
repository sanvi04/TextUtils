...
<button className="btn btn-success mx-3 my-1" onClick={handleUpClick}>Convert to Upper case</button>
<button className="btn btn-success mx-3 my-1" onClick={handleLoClick}>Convert to Lower case</button>
<button className="btn btn-success mx-3 my-1" onClick={handleClear}>Clear Text</button>
<button className="btn btn-success mx-3 my-1" onClick={handleCopy}>Copy Text</button>

    </div>
    <div className="container my-5" style={{color: props.mode==='dark'?'white':'black'}}>
      <h2> Your text summary</h2>
      <p> {text.split(" ").filter((element)=>{return element.length!==0}).length-1} words, {text.length} characters</p>
